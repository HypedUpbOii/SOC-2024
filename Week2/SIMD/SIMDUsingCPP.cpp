...
int main()
{
    ...
    */
    ...
    // OPTIONAL TASK
    alignas(32) int vec1i[130];
    alignas(32) int vec2i[130];
    alignas(32) int vec3i[130];
    __m256i v1i, v2i, v3i; // using 256 bit registers, each float is of 32 bits so each register holds up to 8 floats
    for (int i = 0; i < 130; i++) {
        vec1i[i] = i;
        vec2i[i] = 2 * i;
    }
    ...
    for (int i = 0; i < 130 - 8; i += 8) {
        v1i = _mm256_load_si256(reinterpret_cast<__m256i *>(vec1i + i)); // load vec1[i],vec1[i+1]..vec1[i+7] into 256 bit register
        v2i = _mm256_load_si256(reinterpret_cast<__m256i *>(vec2i + i)); // load vec2[i],vec2[i+1]..vec2[i+7] into 256 bit register
        // Had to compile with mavx2 and not mavx because of the following step :(
        v3i = _mm256_add_epi32(v1i, v2i);                                 // add the two 256 bit registers and store result in a third 256 bit register (v3)
        _mm256_store_si256(reinterpret_cast<__m256i *>(vec3i + i), v3i); // store v3 in vec3+i
    }
    for (int i = 130 - (130 % 8); i < 130; i++) {
        vec3i[i] = vec1i[i] + vec2i[i];
    }
